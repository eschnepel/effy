...
def to_power_equivalent(
    ...
    ``original_unit`` is changed from Wh → W (or kWh → kW) so that
    ``effective_in_original_unit`` inside ``distribute_loss`` converts the
    result back to W/kW — the natural output unit for an average-power value
    over the interval.
