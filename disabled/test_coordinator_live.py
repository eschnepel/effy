"""
=====================================================================
 DISABLED — 2026-07-09. Tests for disabled/coordinator_live.py.

 Not collected by the active test suite (pytest's testpaths is
 tests/ only, at the repo root — see pyproject.toml). Kept here,
 alongside the code it tests, so both can be restored together once
 live-path support is turned back on. Run explicitly if needed:
     pytest disabled/test_coordinator_live.py
=====================================================================

Unit tests for the LiveReading accumulator in the disabled live coordinator.
...
if TYPE_CHECKING:
    from effy.coordinator_live import LiveReading as LiveReading
...
# ---------------------------------------------------------------------------
# 2. Load effy modules by file path — note: this loads the DISABLED
#    coordinator_live.py (this directory), not custom_components/effy's
#    active (reduced) coordinator.py.
# ---------------------------------------------------------------------------

_repo_root = Path(__file__).resolve().parent.parent
_active_base = _repo_root / "custom_components" / "effy"
_disabled_base = Path(__file__).resolve().parent

_effy_pkg = ModuleType("effy")
_effy_pkg.__path__ = [str(_active_base), str(_disabled_base)]
_effy_pkg.__package__ = "effy"
sys.modules["effy"] = _effy_pkg


def _load(reg_name: str, base: Path, filename: str) -> ModuleType:
    path = base / filename
    # No submodule_search_locations here: these are plain modules, not
    # packages — see the identical note in tests/test_coordinator_slot.py.
...
_calc_mod = _load("effy.calculation", _active_base, "calculation.py")
_coord_mod = _load("effy.coordinator_live", _disabled_base, "coordinator_live.py")
...
class TestLiveReadingPower:
    ...
    def test_first_ever_event_is_not_swamped_by_epoch(self) -> None:
        ...
        swamping the new value's weight to
